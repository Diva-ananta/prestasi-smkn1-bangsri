                ...
                <div class="flex min-h-16 items-center justify-between gap-2 py-2 sm:py-2.5">
                    ...
                    <div class="hidden items-center gap-1 rounded-full border border-slate-200 bg-slate-50/70 p-1.5 md:flex dark:border-slate-700 dark:bg-slate-800/60">
                        ...
                        <div x-data="{ publicationOpen: false }" class="relative">
                            <button type="button" @click="publicationOpen = !publicationOpen" class="inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-medium text-slate-600 shadow-sm transition hover:-translate-y-0.5 hover:bg-white hover:text-emerald-700 hover:shadow-md active:translate-y-0 dark:text-slate-300 dark:hover:bg-slate-700 dark:hover:text-emerald-300 {{ request()->routeIs('public.artikel.*', 'public.galeri.*') ? 'bg-emerald-100 text-emerald-800 shadow-sm dark:bg-emerald-950/70 dark:text-emerald-300' : '' }}">Publikasi <x-icon name="chevron-down" class="text-xs" /></button>
            ...
            <div class="relative md:hidden">
                <div x-show="open" x-transition.opacity class="absolute left-2 right-2 top-full z-50 mt-2 rounded-2xl border border-slate-200 bg-white/95 shadow-xl shadow-slate-900/10 dark:border-slate-800 dark:bg-slate-900/95">
                    <div class="mx-auto max-w-7xl space-y-2 px-4 py-4">
                        <a href="/" class="block rounded-xl px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100 dark:text-slate-200 dark:hover:bg-slate-800 {{ request()->routeIs('home') ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-300' : '' }}">Prestasi</a>
                        <a href="{{ route('public.prestasi.index') }}" class="block rounded-xl px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100 dark:text-slate-200 dark:hover:bg-slate-800 {{ request()->routeIs('public.prestasi.*') ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-300' : '' }}">Data Prestasi</a>
                        ...
                        <div x-data="{ publicationOpen: {{ request()->routeIs('public.artikel.*', 'public.galeri.*') ? 'true' : 'false' }} }">
                            <button type="button" @click="publicationOpen = !publicationOpen" class="flex w-full items-center justify-between rounded-xl px-3 py-2 text-left text-sm font-medium text-slate-700 shadow-sm transition hover:-translate-y-0.5 hover:bg-emerald-50 hover:text-emerald-800 hover:shadow-md active:translate-y-0 dark:text-slate-200 dark:hover:bg-slate-800 {{ request()->routeIs('public.artikel.*', 'public.galeri.*') ? 'bg-emerald-50 text-emerald-800 dark:bg-emerald-950/50 dark:text-emerald-300' : '' }}">Publikasi <x-icon name="chevron-down" class="text-xs" /></button>
