...
<div class="grid gap-6 lg:grid-cols-[1fr_300px] lg:items-start">
    ...
    <div class="space-y-6">
        ...
        <div class="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900 md:p-6">
                ...
                <div class="flex max-h-[95vh] w-full max-w-3xl flex-col overflow-hidden rounded-[24px] bg-white shadow-2xl dark:bg-slate-900">
                    ...
                    <div class="border-t border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-900">
                        ...
                        <div class="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
                            <button type="button"
                                    id="cropCancel"
                                    class="inline-flex min-h-10 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-sm font-bold text-slate-600 shadow-sm transition hover:border-slate-300 hover:bg-slate-50 hover:text-slate-800 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 dark:hover:bg-slate-800">
                                Batal
                            </button>
        ...
        <div class="flex flex-col-reverse justify-end gap-3 sm:flex-row">
            <a href="{{ route('admin.prestasi.index') }}" class="inline-flex min-h-10 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-sm font-bold text-slate-600 shadow-sm transition hover:border-slate-300 hover:bg-slate-50 hover:text-slate-800 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 dark:hover:bg-slate-800">Batal</a>
            ...
            <div class="overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-700">
                ...
                <div class="p-4">
                    <div class="mb-2 flex flex-wrap gap-1.5">
                        <span id="previewTingkat" class="inline-flex items-center gap-1 rounded-full bg-emerald-100 px-2 py-1 text-[10px] font-bold uppercase tracking-wide text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-300">{{ old('tingkat', $prestasi->tingkat ?? 'Umum') }}</span>
                        <span id="previewKategori" class="inline-flex items-center gap-1 rounded-full bg-slate-100 px-2 py-1 text-[10px] font-bold uppercase tracking-wide text-slate-600 dark:bg-slate-800 dark:text-slate-300">{{ old('kategori', $prestasi->kategori ?? 'Umum') }}</span>
                    </div>
                    <p id="previewJudul" class="min-h-[2.5rem] text-sm font-black leading-snug text-slate-900 dark:text-white">{{ old('nama_lomba', $prestasi->nama_lomba ?? 'Judul prestasi akan tampil di sini') }}</p>
                    <p id="previewTim" class="mt-2 truncate text-xs font-semibold text-blue-700 dark:text-blue-400 {{ old('nama_tim', $prestasi->nama_tim ?? '') ? '' : 'hidden' }}"><i class="fas fa-users mr-1"></i><span>{{ old('nama_tim', $prestasi->nama_tim ?? '') }}</span></p>
                    <p id="previewTanggal" class="mt-2 flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400 {{ old('tanggal_mulai', $dateValue($prestasi->tanggal_mulai ?? null)) ? '' : 'hidden' }}"><i class="fas fa-calendar text-emerald-600 dark:text-emerald-400"></i><span>{{ old('tanggal_mulai', $dateValue($prestasi->tanggal_mulai ?? null)) }}</span></p>
                    <div class="mt-3 flex items-center justify-between gap-2 border-t border-slate-100 pt-3 text-[11px] text-slate-500 dark:border-slate-800 dark:text-slate-400">
                        <span id="previewJenis">{{ old('jenis_peserta', $prestasi->jenis_peserta ?? 'Individu') }}</span>
                        <span id="previewStatus" class="font-semibold {{ old('status', $prestasi->status ?? 'Draft') === 'Publish' ? 'text-emerald-600' : 'text-slate-400' }}"><i class="fas fa-circle mr-1 text-[6px]"></i>{{ old('status', $prestasi->status ?? 'Draft') }}</span>
                    </div>
                </div>
            </div>
...
<script>
    (() => {
        ...
        const previewTim = document.getElementById('previewTim');
        ...
        document.getElementById('tingkat')?.addEventListener('change', function () {
            previewTingkat.textContent = this.value || 'Umum';
        });
        document.getElementById('kategori')?.addEventListener('change', function () {
            previewKategori.textContent = this.value || 'Umum';
        });
        document.getElementById('nama_tim')?.addEventListener('change', function () {
            previewTim.querySelector('span').textContent = this.value;
            previewTim.classList.toggle('hidden', !this.value);
        });
        document.getElementById('tanggal_mulai')?.addEventListener('change', function () {
            previewTanggal.querySelector('span').textContent = this.value;
            previewTanggal.classList.toggle('hidden', !this.value);
        });
        document.getElementById('jenis_peserta')?.addEventListener('change', function () {
            previewJenis.textContent = this.value || 'Individu';
        });
