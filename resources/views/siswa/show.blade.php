        ...
        <div class="grid items-stretch gap-5 md:grid-cols-[300px_minmax(0,1fr)]">
            ...
            <section>
                <div class="mb-4 flex items-center justify-between border-b border-slate-200 pb-3 dark:border-slate-700">
                    <h2 class="flex items-center gap-2 text-base font-bold text-slate-800 dark:text-white"><x-icon name="trophy" class="text-emerald-700 dark:text-emerald-400" /> Riwayat Prestasi</h2>
                    <span class="rounded-full bg-emerald-100 px-3 py-1 text-xs font-bold text-emerald-800 dark:bg-emerald-950/50 dark:text-emerald-300">{{ $prestasi->count() }} prestasi</span>
                </div>
                    ...
                    @forelse($prestasi as $index => $item)
                        ...
                        <article class="animate-fade-in rounded-xl border border-slate-200 bg-white p-4 shadow-sm transition hover:border-emerald-300 hover:shadow-md dark:border-slate-800 dark:bg-slate-900" style="animation-delay: {{ $index * 80 }}ms">
                            <div class="flex gap-3">
                                <div class="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg bg-slate-100 text-emerald-700 dark:bg-slate-800 dark:text-emerald-400"><x-icon name="award" /></div>
                                <div class="min-w-0 flex-1">
                                    <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                                        <div class="min-w-0">
                                            <div class="mb-1.5 flex flex-wrap gap-1.5">
                                                <span class="rounded-full bg-slate-100 px-2 py-0.5 text-xs text-slate-600 dark:bg-slate-800 dark:text-slate-300">{{ $item->hasil }}</span>
                                                <span class="rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-semibold text-emerald-700 dark:bg-emerald-950/50 dark:text-emerald-300">{{ $item->tingkat ?? 'Umum' }}</span>
                                            </div>
                                        ...
                                        <div class="shrink-0 text-left sm:text-right"><p class="text-xs text-slate-400">Tahun</p><p class="text-sm font-bold text-slate-800 dark:text-white">{{ $year }}</p></div>
                                    </div>
                                    <a href="{{ route('public.prestasi.show', $item->public_token) }}" class="mt-2 inline-flex items-center gap-1.5 text-xs font-bold text-emerald-700 hover:underline dark:text-emerald-400">Lihat detail <x-icon name="arrow-right" class="text-xs" /></a>
                                </div>
                            </div>
                        </article>
                    @empty
                        <div class="rounded-xl border border-dashed border-slate-300 bg-white p-10 text-center dark:border-slate-700 dark:bg-slate-900"><x-icon name="inbox" class="text-3xl text-slate-300 dark:text-slate-600" /><p class="mt-3 text-sm font-semibold text-slate-600 dark:text-slate-400">Belum ada prestasi</p></div>
                    @endforelse
                </div>
                ...
                @if($artikel->isNotEmpty())
                    <div class="mt-8 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                        <h2 class="flex items-center gap-2 text-base font-bold text-slate-900 dark:text-white"><x-icon name="newspaper" class="text-emerald-600" /> Artikel Terkait</h2>
