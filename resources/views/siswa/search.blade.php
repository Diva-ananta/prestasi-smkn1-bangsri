    ...
    <section class="relative overflow-hidden border-b border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
        ...
        <div class="relative mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8 lg:py-12">
            <div class="grid items-stretch gap-6 lg:grid-cols-2">
                ...
                <p class="mt-5 max-w-2xl text-base leading-7 text-slate-600 dark:text-slate-300">Temukan siswa dengan riwayat prestasi publik berdasarkan nama, NIS, kelas, program keahlian, dan angkatan.</p>
                <form id="siswa-search" action="{{ route('public.siswa.search') }}" method="GET" class="mt-5 flex flex-col gap-2 sm:flex-row">
                    ...
                    <button type="submit" class="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-600 px-5 py-3 text-sm font-bold text-white shadow-md shadow-emerald-600/20 transition hover:-translate-y-0.5 hover:bg-emerald-700 hover:shadow-lg active:translate-y-0 active:shadow-md sm:w-auto"><x-icon name="search" />Cari siswa</button>
                </form>
                <a href="{{ route('public.prestasi.index') }}" class="mt-3 inline-flex max-w-full items-center gap-2 self-start rounded-xl border border-emerald-200 bg-emerald-50 px-3.5 py-2.5 text-sm font-bold text-emerald-700 shadow-sm transition hover:-translate-y-0.5 hover:bg-emerald-100 hover:shadow-md active:translate-y-0 dark:border-emerald-800 dark:bg-emerald-950/30 dark:text-emerald-300 dark:hover:bg-emerald-950/50"><x-icon name="trophy" /><span class="truncate">Lihat semua prestasi</span><x-icon name="arrow-right" class="shrink-0 text-xs" /></a>
                </div>
            ...
            @forelse($siswas as $siswa)
                ...
                <article class="group flex h-full flex-col rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition duration-300 hover:-translate-y-1 hover:border-emerald-300 hover:shadow-lg dark:border-slate-800 dark:bg-slate-900 dark:hover:border-emerald-700 animate-fade-in">
                    ...
                    <dl class="mt-4 grid grid-cols-2 gap-2.5 border-t border-slate-100 pt-3 text-sm dark:border-slate-800 sm:text-xs">
                        ...
                        <div><dt class="text-slate-400">Program Keahlian</dt><dd class="mt-1 break-words font-semibold text-slate-700 dark:text-slate-200">{{ $siswa->jurusan ?: '-' }}</dd></div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function createSiswaChart(id, labels, data, color, unit) {
            ...
            const validData = (labels || []).reduce(function (result, label, index) {
                const normalizedLabel = String(label ?? '').trim();
                if (normalizedLabel && normalizedLabel !== '-') {
                    result.labels.push(normalizedLabel);
                    result.data.push(Number(data?.[index] ?? 0));
                }
                return result;
            }, { labels: [], data: [] });
