        ...
        <div class="relative mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8 lg:py-12">
            <div class="grid items-stretch gap-6 lg:grid-cols-2">
                ...
                <form id="siswa-search" action="{{ route('public.siswa.search') }}" method="GET" class="mt-5 flex flex-col gap-2 sm:flex-row">
                    ...
                    <button type="submit" class="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-600 px-5 py-3 text-sm font-bold text-white transition hover:bg-emerald-700 sm:w-auto"><x-icon name="search" />Cari siswa</button>
                </form>
                ...
                <div id="siswa-analytics" x-data="{ activeChart: 'jurusan', selectChart(name) { this.activeChart = name; requestAnimationFrame(() => window.dispatchEvent(new Event('resize'))); } }" class="min-w-0 rounded-[20px] border border-slate-200 bg-white p-4 shadow-lg dark:border-slate-800 dark:bg-slate-900 sm:rounded-[24px] sm:p-5">
                    ...
                    <div class="mt-4 grid grid-cols-3 gap-1.5 border-t border-slate-100 pt-4 dark:border-slate-800 sm:gap-2"><button type="button" @click="selectChart('jurusan')" :class="activeChart === 'jurusan' ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300'" class="rounded-lg px-1 py-2 text-xs font-bold transition sm:px-2 sm:text-xs">Jurusan</button><button type="button" @click="selectChart('angkatan')" :class="activeChart === 'angkatan' ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300'" class="rounded-lg px-1 py-2 text-xs font-bold transition sm:px-2 sm:text-xs">Angkatan</button><button type="button" @click="selectChart('status')" :class="activeChart === 'status' ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300'" class="rounded-lg px-1 py-2 text-xs font-bold transition sm:px-2 sm:text-xs">Status</button></div>
            </div>
        </div>
    </section>
        ...
        <div class="grid items-stretch gap-4 md:grid-cols-[210px_minmax(0,1fr)]">
                ...
                <div class="mb-3 flex items-center justify-between"><h3 class="text-sm font-bold text-slate-900 dark:text-white">Filter</h3><button type="button" @click="filtersOpen = false" class="inline-flex h-8 w-8 items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 md:hidden dark:hover:bg-slate-800" aria-label="Tutup filter"><x-icon name="times" /></button></div>
                    <form id="siswa-filter" action="{{ route('public.siswa.search') }}" method="GET" class="space-y-3">
                        <div class="border-t border-slate-200 pt-3 dark:border-slate-700"><label class="text-xs font-bold text-slate-900 dark:text-white">Jurusan</label><select name="jurusan" class="mt-1.5 w-full rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-xs dark:border-slate-700 dark:bg-slate-800 dark:text-white"><option value="">Semua jurusan</option>@foreach($jurusanOptions as $option)<option value="{{ $option }}" @selected(request('jurusan') === $option)>{{ $option }}</option>@endforeach</select></div>
                        ...
                        <div class="border-t border-slate-200 pt-3 dark:border-slate-700"><label class="text-xs font-bold text-slate-900 dark:text-white">Status</label><select name="status" class="mt-1.5 w-full rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-xs dark:border-slate-700 dark:bg-slate-800 dark:text-white"><option value="">Semua status</option><option value="Aktif" @selected(request('status') === 'Aktif')>Aktif</option><option value="Alumni" @selected(request('status') === 'Alumni')>Alumni</option></select></div>
                        <a href="{{ route('public.siswa.search') }}" data-ajax-filter-reset class="block w-full rounded-lg border border-slate-300 bg-white px-2.5 py-1.5 text-center text-xs font-bold text-slate-700 transition hover:bg-slate-50 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700">Reset filter</a>
                    </form>
                </aside>
            </div>

            <div id="siswa-results">
                <div class="mb-5 flex items-center justify-between rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-sm dark:border-slate-800 dark:bg-slate-900"><p class="text-sm font-semibold text-slate-700 dark:text-slate-300">Ditemukan <span class="font-black text-emerald-700 dark:text-emerald-400">{{ number_format($siswas->total()) }}</span> siswa</p><span class="rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-bold uppercase tracking-[0.18em] text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300">Publish</span></div>
                @if($keyword !== '' || request()->hasAny(['jurusan', 'kelas', 'angkatan', 'status']))
                    <div role="status" aria-live="polite" class="mb-5 flex items-start gap-3 rounded-xl border {{ $siswas->total() ? 'border-emerald-200 bg-emerald-50 text-emerald-800 dark:border-emerald-900/60 dark:bg-emerald-950/30 dark:text-emerald-200' : 'border-amber-200 bg-amber-50 text-amber-800 dark:border-amber-900/60 dark:bg-amber-950/30 dark:text-amber-200' }} px-4 py-3 text-sm">
                        <x-icon :name="$siswas->total() ? 'circle-check' : 'circle-info'" class="mt-0.5 shrink-0" />
            ...
            @forelse($siswas as $siswa)
                ...
                <article class="group flex h-full flex-col rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition duration-300 hover:-translate-y-1 hover:border-emerald-300 hover:shadow-lg dark:border-slate-800 dark:bg-slate-900 dark:hover:border-emerald-700 animate-fade-in">
                    <div class="flex items-start gap-3">
                        <div class="flex h-20 w-20 shrink-0 items-center justify-center overflow-hidden rounded-2xl bg-emerald-100 text-2xl font-black text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300">
                            @if($siswa->foto)<img src="{{ asset('storage/' . $siswa->foto) }}" alt="Foto {{ $siswa->nama }}" class="h-full w-full object-cover">@else{{ $initials }}@endif
                        </div>
                        <div class="min-w-0"><h2 class="break-words text-lg font-bold text-slate-900 dark:text-white sm:text-base">{{ $siswa->nama }}</h2><p class="mt-1 text-sm text-slate-500 dark:text-slate-400 sm:text-xs">NIS: {{ $siswa->nis ?: '-' }}</p></div>
                    </div>
                    <dl class="mt-4 grid grid-cols-2 gap-2.5 border-t border-slate-100 pt-3 text-sm dark:border-slate-800 sm:text-xs">
                    ...
                    <div class="mt-auto flex items-center justify-between gap-3 border-t border-slate-100 pt-3 dark:border-slate-800"><span class="text-sm font-semibold sm:text-xs {{ $siswa->status === 'Aktif' ? 'text-emerald-700 dark:text-emerald-400' : 'text-amber-700 dark:text-amber-400' }}">{{ $siswa->status }}</span><a href="{{ route('public.siswa.profile', ['nama' => \Illuminate\Support\Str::slug($siswa->nama)]) }}" class="inline-flex items-center gap-2 text-sm font-bold text-emerald-700 transition group-hover:gap-3 dark:text-emerald-400">Profil <x-icon name="arrow-right" class="text-xs" /></a></div>
                </article>
            @empty
                <div class="col-span-full rounded-2xl border-2 border-dashed border-slate-300 p-12 text-center dark:border-slate-700"><x-icon name="user-slash" class="text-4xl text-slate-400" /><p class="mt-4 text-lg font-bold text-slate-700 dark:text-slate-300">Belum ada siswa yang sesuai</p><p class="mt-2 text-sm text-slate-500 dark:text-slate-400">Coba ubah kata kunci atau filter yang dipilih.</p></div>
            @endforelse
                </div>
