        ...
        <div class="grid items-stretch gap-4 md:grid-cols-[210px_minmax(0,1fr)]">
            ...
            @forelse($siswas as $siswa)
                ...
                <article class="group flex h-full flex-col rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition duration-300 hover:-translate-y-1 hover:border-emerald-300 hover:shadow-lg dark:border-slate-800 dark:bg-slate-900 dark:hover:border-emerald-700 animate-fade-in">
                    ...
                    <div class="mt-auto flex items-center justify-between gap-3 border-t border-slate-100 pt-3 dark:border-slate-800"><span class="text-sm font-semibold sm:text-xs {{ $siswa->status === 'Aktif' ? 'text-emerald-700 dark:text-emerald-400' : 'text-amber-700 dark:text-amber-400' }}">{{ $siswa->status }}</span><a href="{{ route('public.siswa.show', ['token' =>$siswa->public_token]) }}" class="inline-flex items-center gap-2 text-sm font-bold text-emerald-700 transition group-hover:gap-3 dark:text-emerald-400">Profil <x-icon name="arrow-right" class="text-xs" /></a></div>
                </article>
            @empty
